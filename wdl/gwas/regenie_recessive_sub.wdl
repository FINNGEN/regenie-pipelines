task step2 {
    ...
    String prefix = sub("R8_sisuv4_GRM_V0_LD_0.1_", "_pred.list", "") + pheno + "." + basename(bgen) 
    Array[File] loco
    Array[File] nulls
    Int bsize
    String options
    ...
    command <<<
        ...
        echo "${pheno} /cromwell_root/R8_sisuv4_GRM_V0_LD_0.1.${pheno}.loco.gz" > "R8_sisuv4_GRM_V0_LD_0.1.${pheno}.pred.list"
        echo "${pheno} /cromwell_root/R8_sisuv4_GRM_V0_LD_0.1.${pheno}.firth.gz" >"R8_sisuv4_GRM_V0_LD_0.1.${pheno}.firth.list"
        ...
        --phenoFile ${cov_pheno} \
        --phenoColList ${pheno} \
        --pred R8_sisuv4_GRM_V0_LD_0.1.${pheno}.pred.list \
        ${if is_binary then "--use-null-firth R8_sisuv4_GRM_V0_LD_0.1.${pheno}.firth.list" else ""} \
        --bsize ${bsize} \
        --threads $n_cpu \
        --gz \
        --out ${prefix} \
...
workflow regenie_step2 {
    ...
    scatter (bgen in bgens) {
        call step2 {
            input: docker=docker, pheno=pheno, is_binary=is_binary, cov_pheno=cov_pheno, covariates=covariates, loco=loco, nulls=nulls,bgen=bgen
        }
    }
