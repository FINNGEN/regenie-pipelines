task step2 {
    ...
    runtime {
        docker: "${docker}"
        cpu: if length(phenos) == 1 then 1 else if length(phenos) <=4 then 2 else if length(phenos) <= 10 then 4 else if length(phenos) < 16 then 8 else 16
        memory: if length(phenos) <= 2 then "4 GB" else "6 GB"
...
workflow regenie_step2 {
    ...
    scatter (bgen in bgens) {
        call step2 {
            input: docker=docker, phenos=phenos, is_binary=is_binary, cov_pheno=cov_pheno, covariates=covariates, loco=loco, nulls=nulls,bgen=bgen
        }
    }
